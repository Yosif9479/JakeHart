...
public class GameManager : MonoBehaviour
{
    ...
    {
        return (GlobalValue.SavedCoins >= continueCoinCost) || (UnityAds.Instance && UnityAds.Instance.isRewardedAdReady());
    }
    ...
    void Awake()
    {
        ...
        List<GameObject> allCheckpoints;
        var checkpointGroup = GameObject.Find("Checkpoint Group");
        allCheckpoints = new List<GameObject>(checkpointGroup.transform.childCount);
        for (int i = 0; i < checkpointGroup.transform.childCount; i++)
        {
            if (i == 0)
            {
                allCheckpoints.Add(GameObject.Find("checkpoint"));
                GameObject.Find("checkpoint").AddComponent<CheckpointNumber>();
                GameObject.Find("checkpoint").GetComponent<CheckpointNumber>().checkpointIndex = i;
            }
            else
            {
                allCheckpoints.Add(GameObject.Find($"checkpoint ({i})"));
                GameObject.Find($"checkpoint ({i})").AddComponent<CheckpointNumber>();
                GameObject.Find($"checkpoint ({i})").GetComponent<CheckpointNumber>().checkpointIndex = i;
            }
            Debug.Log("232322222222222");
        }
        if (GlobalValue.lastOrNewLevel == 1)
        {
            var startPoint = GameObject.FindGameObjectWithTag("StartPoint");
            if (startPoint)
                Player.transform.position = startPoint.transform.position;

            if (GlobalValue.platformControllerSwitcher == 1)
                PlatformControllerSwitcher.Instance.AfterTakeDamage();

            if (GlobalValue.levelPlaying == 2 && GlobalValue.hasKeyForSecondLevel == 1)
                TheGate.Instance.UnLockedGate();
        }
        else if(GlobalValue.lastOrNewLevel == 0)
        {
            var startPoint = GameObject.FindGameObjectWithTag("StartPointNew");
            if (startPoint)
                Player.transform.position = startPoint.transform.position;
        ...
        else if(GlobalValue.levelPlaying == 2 && GlobalValue.lastOrNewLevel == 3)
        {
            var startPoint = allCheckpoints[GlobalValue.checkpointNumber];
            if (startPoint)
                Player.transform.position = startPoint.transform.position;
